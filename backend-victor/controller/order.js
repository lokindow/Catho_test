class OrderController{
    ...
    */
    ...
    list(req, res, next){
        ...
        that.orderModel.list().then((data) => {
            if(data.length == 0){
      ...
      /**
    *   @description: Controller responsavel pela insert
    *   @param: req, res, next (parametros padrões do restify)
    *   @return: Object retorno padrão {message : string, erro : object, retorno : object}
    */
    ...
    insert(req, res, next){
        ...
        that.orderModel.insert(param).then((data) => {
            that.orderModel.consult([data.insertId]).then((data) => {
                res.json({ message: 'Order cadastrado com sucesso', erro: [], retorno: data });
                next();
                return;
            }).catch((data) => {
